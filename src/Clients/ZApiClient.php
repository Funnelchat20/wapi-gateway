...
class ZApiClient implements MessagesContract, InstancesContract, GroupsContract, ContactsContract, QueueContract
{
    ...
    {
        $startTime = microtime(true);
        $url = str_replace(['UID', 'TOKEN', 'ACTION'], [$uid, $token, 'extension-token'], $this->baseUrl());
    ...
    {
        $url = str_replace(['UID', 'TOKEN', 'ACTION'], [$uid, $token, 'update-webhook-received'], $this->baseUrl());
        $res = Http::withHeaders(['Client-Token' => config("$this->configPrefix.client_token")])
    ...
    {
        $url = str_replace(['UID', 'TOKEN', 'ACTION'], [$uid, $token, 'update-webhook-received-and-delivery'], $this->baseUrl());
        $res = Http::withHeaders(['Client-Token' => config("$this->configPrefix.client_token")])
    ...
    {
        $startTime = microtime(true);
        $url = str_replace(['UID', 'TOKEN', 'ACTION'], [$uid, $token, 'update-community-description'], $this->baseUrl());
    ...
    {
        $startTime = microtime(true);
        $url = str_replace(['UID', 'TOKEN', 'ACTION'], [$uid, $token, 'communities/' . $communityId], $this->baseUrl());
        $res = Http::withHeaders(['accept' => 'application/json', 'Client-Token' => config("$this->configPrefix.client_token")])->delete($url);
    ...
    public function updateNewsletterName(string $uid, string $token, string $id, string $name): array
    {
        ...
        $payload = ['id' => str_ends_with($id, '@newsletter') ? $id : $id . '@newsletter', 'name' => $name];
        $res = Http::withHeaders(['Client-Token' => config("$this->configPrefix.client_token")])->timeout(config("$this->configPrefix.timeout", 120))->post($url, $payload);
    ...
    public function updateNewsletterDescription(string $uid, string $token, string $id, string $description): array
    {
        ...
        $payload = ['id' => str_ends_with($id, '@newsletter') ? $id : $id . '@newsletter', 'description' => $description];
        $res = Http::withHeaders(['Client-Token' => config("$this->configPrefix.client_token")])->timeout(config("$this->configPrefix.timeout", 120))->post($url, $payload);
    ...
    public function updateNewsletterPicture(string $uid, string $token, string $id, string $photoUrl): array
    {
        ...
        $payload = ['id' => str_ends_with($id, '@newsletter') ? $id : $id . '@newsletter', 'pictureUrl' => $photoUrl];
        $res = Http::withHeaders(['Client-Token' => config("$this->configPrefix.client_token")])->timeout(config("$this->configPrefix.timeout", 120))->post($url, $payload);
